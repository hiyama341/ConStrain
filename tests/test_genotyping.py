...
def test_pairwise_alignment_of_templates():
    ...
    # sequencing reads
    reads = []
    for seq_record in SeqIO.parse('../ConStrain/tests/files_for_testing/sequencing_reads.fasta', format= 'fasta'):
        reads.append(seq_record)    

    # primers
    pad_pG8H_fw = SeqIO.read('../ConStrain/tests/files_for_testing/pad_pG8H_fw.fasta', format = 'fasta')
    pad_pCPR_fw = SeqIO.read('../ConStrain/tests/files_for_testing/pad_pCPR_fw.fasta', format = 'fasta')
