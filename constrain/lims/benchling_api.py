...
def sequence_to_benchling(folder_name, oligo_name, oligo_bases, schema):
    ...
    schemas = ('Primer', 'DNA Fragment', 'Plasmid', 'Gene', 'gRNA', 'Marker', 'Promoter', 'Terminator', 'Tag', 'Origin of Replication')
...
def from_benchling(bname, schema=''):
    ...
    translated_bench_dict_sel, translated_bench_dict_other = split_based_on_keys(trans_bench_dict, trans_dict.values())
    
    ## Add Benchling custom fields and isCircular to fields dict in Biopython annotations.
    translated_bench_dict_sel['annotations'].update(translated_bench_dict_other['customFields'])
    translated_bench_dict_sel['annotations'].update({'topology': translated_bench_dict_other['isCircular']})
    ...
    translated_bench_dict_sel['annotations'].update({'data_file_division': "PLN",
                                                     'date': date,
                                                     'molecule_type': 'DNA',
                                                     'location' : 'unknown',
                                                     'commentary':comment})
    ...
    translated_bench_dict_sel['features'] = [SE_to_CLoc(feature_dict, seq_length) for feature_dict in translated_bench_dict_sel['features']]
    ...
    translated_bench_dict_sel['features'] = [nest_dict(feature_dict, first_order_keys = ['location', 'type', 'strand'], key_for_nested_dict = 'qualifiers') for feature_dict in translated_bench_dict_sel['features']]
    
    
    #### Create SeqFeatures
    translated_bench_dict_sel['features'] = [Bio.SeqFeature.SeqFeature(**feature_dict) for feature_dict in translated_bench_dict_sel['features']]
...
def update_loc_vol_conc(seqRecord):
    ...
    for i, row in DB.iterrows():
        if row['batchEntId'] == seqRecord.id:
            ...
            seqRecord.annotations['batches'].append( {'box'          : row['parentBoxPlateName'],
...
def nest_dict(dictionary, key_for_nested_dict, first_order_keys = None):
    ...
    first_order_dict, second_order_dict = split_based_on_keys(dictionary, first_order_keys)
